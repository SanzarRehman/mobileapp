...
import com.example.grpc.common.EventHandlingServiceGrpc;
import com.example.grpc.common.SubmitEventRequest;
import com.example.grpc.common.SubmitEventResponse;
...
public class CustomServerEventPublisher {
    ...
    public CustomServerEventPublisher(
        ...
        @Value("${app.custom-server.url:http://localhost:8081}") String customServerUrl, @Value("${app.grpc-server.url:localhost}")String masterGrpcServerUrl, @Value("${app.grpc-server.port:9060}")int masterGrpcServerPort, ObjectMapper objectMapper) {
