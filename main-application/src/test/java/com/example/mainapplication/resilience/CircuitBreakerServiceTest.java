...
class CircuitBreakerServiceTest {
    ...
    @Test
    void executeWithCircuitBreaker_RunnableFailingOperation_ShouldPropagateException() {
        // Given
        String serviceName = "failing-runnable-service";
        RuntimeException expectedException = new RuntimeException("Runnable failure");
    ...
    @Test
    void executeWithCircuitBreaker_SameServiceMultipleTimes_ShouldReuseSameCircuitBreaker() {
        // Given
        String serviceName = "reuse-service";
        AtomicInteger callCount = new AtomicInteger(0);

        // When
        circuitBreakerService.executeWithCircuitBreaker(serviceName, callCount::incrementAndGet);
        circuitBreakerService.executeWithCircuitBreaker(serviceName, callCount::incrementAndGet);
